...
// --- Configuration Data ---
const TOOLS_DATA = [
  { id: 'shadowbreach', icon: 'key', name: 'SHADOWBREACH', desc: 'APT Emulation Suite', details: 'Automate complex, multi-stage attack scenarios for comprehensive testing.', command: 'wolfguider-sb --mode apt-sim --target corp-vms', ip: '192.168.1.10', port_scan: true },
  { id: 'cryptex', icon: 'lock', name: 'CRYPTEX', desc: 'Secure Data Protocol', details: 'Ensure integrity and zero-knowledge transfer across all critical assets.', command: 'wolfguider-cx --protocol secure-pki --cert-deploy', ip: '10.0.0.55', port_scan: false },
  { id: 'pulsar', icon: 'scan', name: 'PULSAR SCAN', desc: 'Vulnerability Detection', details: 'Pinpoint and prioritize systemic weaknesses using proprietary scanning logic.', command: 'wolfguider-ps --scan-type deep-xss --output json', ip: '203.0.113.12', port_scan: true },
];
...
const WolfLogo = ({ className = 'w-32 h-32' }) => (
  <svg
          ...
          <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 sm:gap-8 lg:gap-10">
              {TOOLS_DATA.map((tool) => (
                <div
                  key={tool.id}
                  className={`tool-card p-4 sm:p-6 lg:p-8 rounded-lg sm:rounded-xl border border-gray-800 shadow-lg sm:shadow-2xl shadow-black/70 transition-all duration-500 ease-out ${activeTool?.id !== tool.id ? 'hover:border-red-600 hover:shadow-red-900/70' : 'border-red-500 shadow-red-900/50'}`}
                >
                  <div className="flex items-start mb-4 sm:mb-6 border-b border-red-900/50 pb-3 sm:pb-4">
                    <div className="p-2 sm:p-3 bg-red-900/30 rounded-lg text-red-500 mr-3 sm:mr-4 border border-red-800 transition-all duration-500 group-hover:bg-red-900/50 shadow-[0_0_10px_rgba(239,68,68,0.5)]">
                      <Icon name={tool.icon} className="w-5 h-5 sm:w-6 sm:h-6 lg:w-7 lg:h-7" />
                    </div>
                    <div>
                      <h4
                        className="text-lg sm:text-xl lg:text-2xl font-bold tracking-wider text-red-400 mt-1 cursor-pointer hover:text-red-300 transition-colors font-mono"
                  ...
                  <button
                    onClick={() => setActiveTool(tool)}
                    className="flex items-center text-red-500 font-semibold text-sm sm:text-md hover:text-red-300 transition-all duration-500 border-b border-red-500/0 hover:border-red-500 pb-0.5 font-mono w-full justify-center sm:justify-start"
                  >
                    RUN SIMULATION <span className="ml-2 text-lg sm:text-xl">&raquo;</span>
                  </button>
                </div>
              ))}
            </div>
